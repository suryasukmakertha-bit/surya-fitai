...
export default function ProgressDownloadCard({
  ...
  const generateImage = async (): Promise<Blob> => {
    ...
    ctx.fillText(t.progressReport, W / 2, y);
    ...
    const stats = [
      ...
      [t.programLabel, programName],
      [t.durationLabel, duration],
      [t.weightLabelShort, `${weight} kg`],
      [t.bmiLabel, bmi],
      [t.calorieTargetLabel, `${calorieTarget} kcal`],
    ];
    ...
    if (isComplete) {
      ...
      ctx.textAlign = "center";
      ctx.fillText(t.eliteCompleted, badgeX, badgeY2 + 28);
    }
    ...
    if (isComplete) {
      ...
      ctx.fillText(t.consistency, ringCX, ringCY + ringR + 18);
    }
    ...
    ctx.textAlign = "center";
    ctx.fillText(t.generatedBy, W / 2, H - 90);
