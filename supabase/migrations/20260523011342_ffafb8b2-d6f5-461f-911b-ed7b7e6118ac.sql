-- Add longest_streak to profiles (historical best, monotonically increasing)
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS longest_streak integer NOT NULL DEFAULT 0;

-- Allow the tampering trigger to ignore longest_streak (it's never touched there since
-- the trigger only checks specific generate counters, so nothing to change).

-- RPC to bump longest_streak only if the new value is higher.
CREATE OR REPLACE FUNCTION public.bump_longest_streak(p_value integer)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT longest_streak INTO v_current
    FROM public.profiles WHERE user_id = v_user;

  IF v_current IS NULL THEN
    -- No profile row (edge case) — insert minimal
    INSERT INTO public.profiles (user_id, longest_streak)
    VALUES (v_user, p_value)
    ON CONFLICT (user_id) DO UPDATE
      SET longest_streak = GREATEST(public.profiles.longest_streak, EXCLUDED.longest_streak);
    RETURN p_value;
  END IF;

  v_new := GREATEST(v_current, p_value);
  IF v_new <> v_current THEN
    UPDATE public.profiles
      SET longest_streak = v_new, updated_at = now()
...
-- Update medal award function to validate STREAK_* medals against profile.longest_streak
...
BEGIN
  ...
  ELSIF p_medal_id IN ('STREAK_3','STREAK_7','STREAK_30') THEN
    -- Use historical best streak stored on the profile (monotonic)
